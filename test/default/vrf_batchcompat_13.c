#define TEST_NAME "vrf_batchcompat_13"
#include "cmptest.h"
...
int main(void)
{
    unsigned char *seed;
    char pk_hex[32 * 2 + 1];
    char proof_hex[128 * 2 + 1];
    char output_hex[64 * 2 + 1];

    unsigned char sk[64];
    unsigned char pk[32];
    unsigned char proof[128];
    ...
    for (i = 0U; i < (sizeof test_data) / (sizeof test_data[0]); i++) {
        ...
        if (crypto_vrf_ietfdraft13_prove_batchcompat(proof, sk, messages[i], i) != 0){
            printf("crypto_vrf_prove() error: [%u]\n", i);
        }
        printf("%s\n", sodium_bin2hex(proof_hex, sizeof proof_hex, proof, sizeof proof));

        if (crypto_vrf_ietfdraft13_verify_batchcompat(output, pk, proof, messages[i], i) != 0){
        ...
        if (i > 0) {
            if (crypto_vrf_ietfdraft13_verify_batchcompat(output, pk, proof, messages[i], i-1) == 0){
                printf("verify succeeded with truncated message: [%u]\n", i);
            }
        }

        if (crypto_vrf_ietfdraft13_proof_to_hash_batchcompat(output, proof) != 0){
